<?php
$this->title = "Création / modification";
...
<div class="row justify-content-md-center">
    <div class="col-9">
        <h4 class="text-center text-md-start"><?= $editTitle ?></h4>
        <form method="post" action="../public/index.php?route=<?= $route; ?>">
            <div class="row">
                <div class="col">
                    <div class="mb-3">
                        <label class="form-label" for="title">Titre de l'article</label>
                        <input type="text" class="form-control" id="titleContent" name="title" aria-describedby="titleContentHelp" value="<?= isset($post) ? htmlspecialchars($post->get('title')): ''; ?>">
                    ...
                    <div class="mb-3">
                        ...
                        <label class="form-label" for="title">Nom de l'auteur :</label>
                        <input type="text" class="form-control" id="authorPseudo" name="authorPseudo"value="<?= isset($post) ? htmlspecialchars($post->get('authorPseudo')): ''; ?>"  disabled>
                        <input type="hidden" class="form-control" id="authorId" name="authorId"value="<?= isset($post) ? htmlspecialchars($post->get('authorId')): ''; ?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label" for="category">Catégorie de l'article : </label>
                        <input type="text" class="form-control" id="category" name="category"value="<?= isset($post) ? htmlspecialchars($post->get('category')): ''; ?>"  disabled>
                    </div>
                    <div class="mb-3">
                        <label class="form-label" for="lede">Chapô :</label>
                        <textarea class="form-control" id="ledeContent" name="lede" aria-describedby="ledeContentHelp" style="height: 120px"><?= isset($post) ? htmlspecialchars($post->get('lede')): ''; ?></textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="contentContent" name="content" aria-describedby="contentContentHelp" style="height: 500px"><?= isset($post) ? htmlspecialchars($post->get('content')): ''; ?></textarea>
